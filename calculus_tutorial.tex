...
\begin{abstract}
	...
	This material is normally taught in two separated university courses CALC1 and CALC2,
	but we choose to present all topics together to highlight the connections between them.
	The explanations in this tutorial combine words,
	math formulas, visualizations and Python code examples.
	Calculus techniques are used in businesses, engineering, and science.
...
%		\input{statsprobs/chapter21_integrals_fEQc_and_gEQmx.tex}
	...
	\subsection{Problem 3: integration using substitution}
		...
		\paragraph{Solution}
			...
			which conveniently contains $x$ that appears in the faction numerator,
			...
			The lower limit $x = 0$ became $u = 1 + 0^2 = 1$,
			and the upper limit $x = 1$ becomes $u = 1 + 1^2 = 2$,
			which the complete substation:
			\[
				\int_{x=0}^{x=1} \frac{4x}{(1+x^2)^3}\,dx
				=
