...
\section{Vector calculus}
	...
	The vector field $\mathbf{F}$ assigns $3$-dimensional output vector $\mathbf{F} = (F_x, F_y, F_z)$
	for each point $\mathbf{r} = (x,y,z)$ in the three dimensional space.
	Note we use boldface symbols to denote vector quantities.
	...
	\subsection{Example: electric field around a positive charge}
		...
		The electric field is strongest (long lines) close to the charge,
		and reduces in strength the further away from the charge we go.
		...
		%
		Electric field are used in the study of electromagnetism.
		Specifically,
		the electric field $\mathbf{E}(x,y,z)$ describes the strength and the direction
		of the \emph{electric force} that a changed particle would experience if placed at $(x,y,z)$.
	...
	\subsection{Vector calculus derivatives}
		...
		and these are defined in terms of the vector derivative operator (\emph{nabla})
		...
		\]
		The divergence tells us if field $\mathbf{F}$
	...
	\subsection{Vector calculus integrals}
		...
		is a a small ``piece of the surface.''
	...
	% PARAMETRIZATION
	The main practical skill you'll learn if you take a vector calculus 
	is how to \emph{parametrize} regions of space.
	This skill is closely related to the substitution trick (change of variables)
	that we learning in Section~\ref{integrals:techniques_of_integration},
