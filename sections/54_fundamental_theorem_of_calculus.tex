%!TEX root = ../calculus_tutorial.tex
	...
	\subsection{Act 3: Fundamental theorem of calculus}
		...
		\noindent
		We confirm that that the sequence of operations
		$\frac{d}{dx} \int_0^x f(u)\,du  = \tt{sp.diff(sp.integrate(}f(x)\tt{))}$
		brings us back to the original $f(x)$ we started from.
		...
		We'll start with the function $\tt{Gx} = G(x) = x^3$,
		...
		\subsubsection{Using derivative formulas in reverse}
			...
			that contains the mix of several functions on the right side of Table~\ref{table:derivatives}.
