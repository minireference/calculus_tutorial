%!TEX root = ../calculus_tutorial.tex
	...
	\subsection{Act 3: Fundamental theorem of calculus}
		...
		We observed a pattern in the formulas for the integral functions $F_0(b)$ and $G_0(b)$.
		The integral function of the constant function $f(x)=3$
		is a linear function $F_0(b) = 3b$.
		The integral of the linear function $g(x) = x$
		is a quadratic function $G_0(b) = \frac{1}{2}b^2$.
		%	The integral of the quadratic polynomial $h(x) = 4 - x^2$
		%	is a third degree polynomial $H_0(b) = 4b - \frac{1}{3}b^3$.
		In both cases,
		the integral function is a polynomial with one degree higher than the original function.
		What is up with that?
		Is this a coincidence,
		or is there some fundamental pattern that applies to all integral functions?
		% we could follow to ``guess'' the formulas of 
		...
		integrals correspond to the \emph{area} calculations,
		whereas derivatives compute \emph{slopes},
		yet behold:

		\begin{shadetheorem}[fundamental theorem of calculus]
			Let $f(x)$  be a continuous function,
			and let $a \in \mathbb{R}$ be a constant.
			Define the integral function $F_a(x)$ as follows:
			\[
		...
		\noindent
		Note we used the variable $u$ inside the integral
		since $x$ is already used to denote the upper limit of integration.

		% MAYBE: 	In words, the FTC tells us that ...

		
		To understand the inverse relationship between integrals and derivatives,
		...
		See Figure~\ref{fig:functions-inverse} (page~\pageref{fig:functions-inverse}).
		...
		you'll get back to the original function:
		\[
			\frac{d}{dx} \int_c^x f(u)\,du = f(x).
		\]
		Let's use SymPy to verify that the fundamental theorem of calculus is true.
		We'll start with the function $\tt{fx} = f(x) = x^2$,
		compute its integral function $\tt{Fx}$ using \tt{sp.integrate},
		then take the derivative of $\tt{Fx}$ using \tt{sp.diff}.
		...
		$\tt{sp.diff(sp.integrate(}f(x)\tt{))} = \frac{d}{dx}\!\int_0^x \!f(u)du$
		brings us back to the original \tt{fx} we started~with.
		...
		Similarly,
		the inverse relationship between integrals and derivatives holds in the other direction too.
		...
		We start with the function $\tt{Gx} = G(x) = x^3$,
		compute its derivative $\tt{dGdx} = G^{\prime\!}(x)$ using \tt{sp.diff},
		then use \tt{sp.integrate} to integrate the function of $G^{\prime\!}(x)$.
		...
		\noindent
		Applying the operations $\tt{integrate(diff(}G(x)\tt{))}\!=\int_0^x\!G^{\prime\!}(u)\,du$
		brings us back to the original \tt{Gx} that we started with.
		...
		\subsubsection{Using antiderivatives to compute integrals}
			...
			since adding any constant $C$ to an antiderivative function
			...
			The fundamental theorem of calculus
			states that antiderivative functions $F(x)+C$
			...
			%
			This equivalence gives us an analytical shortcut
			for obtaining the integral function $F_0(b) = \int_0^b f(x)\,dx$
			...
			Let's use the antiderivative reverse engineering procedure
			to find the integral function $H_0(b)$ in Example~13.
			

			\paragraph{Example 13 continued}

				We're given the function $h(x) = 4 - x^2$
				and we want to find its integral function $H_0(b) = \int_0^b  h(x)\, dx$.
				The fundamental theorem of calculus tells us this problem
				...
				we can guess that antiderivative $H(x)$ must contain an $x^3$ term,
				because taking the derivative of a cubic term results in a quadratic term.
				Therefore,
				the antiderivative function has the form $H(x) = 4x - kx^3$,
				for some multiplicative constant $k$.
				We must choose the constant $k$ that makes this equation true:
				...
				The equivalence between antiderivatives and integral functions
				tells us that the integral function is $H_0(b) =  4b - \frac{1}{3}b^3 + C$
				for some constant $C$.
				We know from the geometric definition of the integral that
				when $b=0$ the integral function must have value zero,
				so $C=0$ in this case.
				The integral function we're looking for is therefore
				$H_0(b) = \int_0^b h(x)\,dx = 4b - \frac{1}{3}b^3$.
		...
		\subsubsection{Using derivative formulas in reverse}
			...
			We can use it to find the integral functions
			for all the functions listed in the table of derivative formulas
			(Table~\ref{table:derivatives} on page~\pageref{table:derivatives}).
			% based on using the derivative formulas in reverse to guess the value of $F(x)$ 
			For example,
			the table tells us that the integral function of an exponential
			...
			Similarly,
			we can guess the integrals of the trigonometric functions
			$\int \cos(x) \,dx = \sin(x)$, since $\big[\!\sin(x)\big]' = \cos(x)$,
			and $\int - \sin(x) \,dx = \cos(x)$, since $\big[\!\cos(x)\big]' = - \sin(x)$.
			We can verify all these integral formulas by computing
			the integral of the function $f(x) = m + e^x + \frac{1}{x} + \cos(x) - \sin(x)$,
			which contains a mix of functions from the right side of~Table~\ref{table:derivatives}.
