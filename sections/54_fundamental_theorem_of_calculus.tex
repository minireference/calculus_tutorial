...
the FTC tells us that ...
		...
		calculate the derivative function $G'(x)$,
		then compute the integral of the derivative function $G'(x)$,
		we arrive back at the original function $G(x)$
		(up to an additive constant):
		\[
			G(x) = \int_c^x G'(u)\;du = G(x) + C.
		\]
		% The constant $C$ 
		...
		To verify $\int_c^x G'(u)\;du = G(x) + C$,
...
For example...
		...
		% Using integrals by reverse engineering derivatives}
			...
			If we can find a function $F(x)$ such that $F'(x) = f(x)$,
...
%then 
			...
			\paragraph{Example 3 continued}
				...
				%	We can rephrase this problem as the search for some function $H(x)$ such that $H'(x) = x^2$.
				...
				for some constant $c$.
				Pick the constant $c$ that makes this equation true: $H'(x) = 4 - 3cx^2 = 4 - x^2$.
				Solving $3c=1$,
...
%	The derivative function $f'(x)$ is defined by the formula
