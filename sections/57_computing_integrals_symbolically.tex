%!TEX root = ../calculus_tutorial.tex
	...
	\subsection{Computing integral functions using SymPy}
		...
		We start by defining three symbols \tt{x}, \tt{a}, and \tt{b}.
		...
		\subsubsection{Example 11S: Constant function}
			...
			that works for all possible limits of integration $a$ and $b$.
			Geometrically,
			we recognize the result \tt{3*(b-a)} as the height-times-width formula
			for the area of a rectangle,
			which we have seen several times already.

			To compute the definite integral $\int_0^5 f(x)\,dx$,
			...
			We'll use the method \tt{subs} (short for substitute)
		...
		\subsubsection{Example 12S: Linear function}
		...
		\subsubsection{Example 13S: Polynomial function}

			We start by defining a SymPy expression
			that corresponds to the function $h(x) = 4 - x^2$.
			...
			\noindent
			We can now call \tt{sp.integrate}
			to make SymPy compute the integral function $H_0(b) = \int_0^b h(x)\,dx$:
			...
			corresponds to the area calculation under $h(x) = 4 - x^2$
			starting at $x=0$.
