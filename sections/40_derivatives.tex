...
\label{sec:derivatives}
	...
	For example,
	the constant function $f(x)=c$ has derivative $f^{\prime\!}(x)=0$ since the function $f(x)$ does not change at all.
	The derivative function describes the \emph{slope} of the graph of the function $f(x)$.
	The derivative of a line $f(x)=mx+b$ is $f^{\prime\!}(x)=m$,
	since the slope of this line is equal to $m$
	for all values of $x$.
	%
	In general,
	the slope of a function is different at different values of~$x$,
	so mathematicians invented a new notation $f^{\prime\!}(x)$
	...
	\begin{figure}[htb]
		\centering
		\includegraphics[width=0.99\columnwidth]{figures/calculus/derivative_as_slope_small-0.pdf}
	...
	$f^{\prime\!}(x)$ is not an independent function---it is \emph{derived} from the original function $f(x)$.

	The \emph{derivative operation}, 
	denoted $\frac{d}{dx}[\tt{<f>}]$,		% ALT. derivative operator $\frac{d}{dx}$
	takes as input a function $f(x)$ and produces as output the derivative function $f^{\prime\!}(x)$.
	...
	Applying the derivative operation to a function
	is also called ``taking the derivative'' of a function.
	%
	For example, 
	the derivative of the function $f(x)=\tfrac{1}{2}x^2$ is the function $f^{\prime\!}(x)=x$.
	...
	that the slope of $f(x)=\tfrac{1}{2}x^2$ is described by $f^{\prime\!}(x)=x$ everywhere on the graph.
	...
	\subsection{Numerical derivative calculations}
		...
		\noindent
		We obtain the approximation $f^{\prime\!}(1) = 1.000000082740371$,
		which is not perfect but pretty close to the true value $f^{\prime\!}(1) = 1$.
	...
	\subsection{Derivative formulas}
		...
		whenever you need to compute $f^{\prime\!}(x)$ in a later calculation.

		Table~\ref{table:derivatives} shows the derivative formulas
		...
		%	F(x)			&  \ - \textrm{ derivative } \to  \quad		F^{\prime\!}(x)		\\
...
%		\alpha f(x)+ \beta g(x)		&\qquad  \raisebox{.52ex}{\rule{0.9em}{.4pt}}\;\tfrac{d}{dx} \rightarrow \qquad 	\alpha f^{\prime\!}(x)+ \beta g^{\prime\!}(x)	\\
	...
	\subsection{Derivative rules}
		...
		\paragraph{Sum rule}
			...
			The derivative is a \emph{linear} operation, which means:
			\[
		...
		\paragraph{Chain rule}
			...
			the value of $f^{\prime}$ evaluated at $g(x)$.
			As the second step,
			we multiply the resulting expression by the derivative of the \emph{inner} function $g^{\prime\!}(x)$.
	...
	\subsection{Higher derivatives}
		...
		has derivative $f^{\prime\!}(x)=x$ and second derivative $f^{\prime\prime\!}(x)=1$,
	...
	\subsection{Applications of derivatives}
		...
		\subsubsection{Tangent lines}
			...
			is $T_1(x) = f(1)  +  f^{\prime\!}(1)(x-1) = \frac{1}{2} + 1\cdot(x-1) = x - \frac{1}{2}$.
	...
	\subsection{Optimization}
		...
		where the slope of the function is zero $f^{\prime\!}(0)=0$.
		Note also the second derivative of is positive at that point $f'^{\prime\!}(0) > 0$,
		...
		\subsubsection{Analytical optimization}
			...
			if the second derivative is positive at that point $f'^{\prime\!}(x^*)>0$ (positive curvature).
			In contrast,
			a critical point $x^*$ here $f'^{\prime\!}(x^*)< 0$ (negative curvature) is a maximum.			
			This observation suggests a general procedure for finding minima and maxima:

			\begin{enumerate}
				\item[(1)]	Solve $f^{\prime\!}(x)=0$ to find the critical points $x^*$.
				\item[(2)]	For each critical point $x^*$,
						check to see if it is a maximum or a minimum
						by evaluating $f'^{\prime\!}(x^*)$:
			...
			\paragraph{Example}
				...
				The second derivative of the function is $f'^{\prime\!}(x) = 6x -4$.
				For the critical value $x^*_1 = \frac{1}{3}$,
				we find $f'^{\prime\!}(\frac{1}{3}) = -2 < 0$,
		...
		\subsubsection{Numerical optimization}
			...
			We can use use this procedure to find the minimum of any function.
			For example,
			let's see what happens when we call \tt{gradient\_descent}
			on the function $q(x)=(x-5)^2$
			...
			\noindent
			The procedure repeats the ``move downhill'' operation several times,
			until it reaches the point $x^*=5.00$,
			which is the minimum of the function $q(x)$.
		...
		\subsubsection{Numerical optimization using SciPy}
			...
			Here is a quick example that shows how to use this procedure
			on the $q(x)=(x-5)^2$.
			...
			\noindent
			The answer is roughly the same as the procedure we defined,
			but the optimization was done much faster.
