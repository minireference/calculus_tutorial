...
\label{sec:derivatives}
	...
	The \emph{derivative} function, denoted $f^{\prime\!}(x)$, $\frac{d}{dx}\big[f(x)\big]$, or $\frac{df}{dx}$,
	...
	Let's start by calculating the \emph{average} slope of the function
	between the points $(x,f(x))$ and $(x+\Delta x, f(x+\Delta x))$ on the graph of the function $f$.
	We denote the horizontal distance that separates the two points as $\Delta x$ (read \emph{delta} x),
	and the vertical distance between the points as $\Delta y = f(x+\Delta x) - f(x)$.
	...
	at $x=1$ for different horizontal distances $\Delta x$.
	When $\Delta x = 2$,
	the average slope is $m = \frac{\Delta y}{\Delta x} = \frac{8}{2} = 4$.
	When $\Delta x = 1$,
	we get $m = \frac{\Delta y}{\Delta x} = \frac{3}{1} = 3$.
	When $\Delta x = 0.3$,
	the slope is $m = \frac{\Delta y}{\Delta x} = \frac{0.69}{0.3} = 2.3$.
	If we continue this process with smaller and smaller $\Delta x$ steps,
	we'll obtain the \emph{instantaneous} slope of the function at the point $x$.

	\begin{figure}[htb]
		\centering
		\includegraphics[width=0.99\columnwidth]{figures/calculus/slope_calculations.pdf}%
		\vspace{-2mm}
		\caption{	Calculating the slope of the line
				that passes through the points $(x,f(x))$
				and $(x+\Delta x, f(x+\Delta x))$
				for three values of the step size $\Delta x$.}
	...
	In words,
	this derivative formula is the rise-over-run calculation
	for an infinitely short horizontal distance $\delta$.
	
	The derivative is a function of the form $f^{\prime\!}: \mathbb{R} \to \mathbb{R}$.
	It takes the value $x$ as input and tells you the slope of the function $f$ at that value.
	...
	% MAYBE: explain x=1 is the limit of what we saw with \Delta x argument then discuss x=-0.5 separately
	...
	\begin{figure}[htb]
		...
		\caption{	The derivative of the function at $x=a$
				...
				and describes the slope of the function at that point.
	...
	The derivative function $f^{\prime\!}(x)$ is a property of $f(x)$.
	Indeed,
	this is where the name \emph{derivative} comes from:
	...
	% which is a function of the form $f^{\prime\!}: \mathbb{R} \to \mathbb{R}$.
	...
	is called ``taking the derivative'' of the function.
	For example, 
	the derivative of the function $f(x) = x^2$ is the function $f^{\prime\!}(x) = 2x$.
	We can also write this as $\big[x^2\big]^{\prime} = 2x$
	or as $\tfrac{d}{dx}\big[x^2\big] = 2x$.
	...
	For example,
	when $x=0$,
	the graph has zero slope
	and the derivative $f^{\prime\!}(x) = 2x$
	is also zero $f^{\prime\!}(0) = 0$.
...
\vspace{-4mm}	% LAYOUT
	...
	\subsection{Numerical derivative calculations}
		...
		a numerical approximation to the derivative of the function \tt{f} at the point~\tt{x}:
		...
		instead of the infinitely small step $\delta$
		like in the math definition of $f^{\prime\!}(x)$.
		This means the value returned by \tt{differentiate}
		is an approximation to the true value of the derivative,
		but it is a fairly accurate one.
		...
		%	by simply evaluating the derivative function at that point:
...
\vspace{-4mm}	% LAYOUT
	...
	\subsection{Derivative formulas}
		...
		it's enough to use the limit formula once
		and record the result you obtain for $f^{\prime\!}(x)$,
		then you can reuse that result
		whenever you need to compute $f^{\prime\!}(x)$ in later calculations.

		Table~\ref{table:derivatives} shows the derivatives of the key functions
...
\vspace{-2mm}
		...
		there are some important derivative rules
		...
		\paragraph{Chain rule}
			...
			The second step
			is to multiply this expression by the derivative of the \emph{inner} function $g^{\prime\!}(x)$.



	\subsection{Examples}

		Armed with the derivative formulas from Table~\ref{table:derivatives}
		and the derivative rules from the previous section,
		we can find the derivative of any function,
	...
	\subsection{Higher derivatives}
		...
		denoted $f^{\prime\prime\!}(x)$ or $\frac{d^2f}{dx^2}$,
		is obtained by applying the derivative operation \emph{twice}:
		$\frac{d}{dx}\big[ \frac{d}{dx}[f(x)] \big]$.
		% Geometrically,
	...
	\subsection{Computing derivatives using SymPy}
		...
		\noindent
		Here is an example of the derivative of a function
		that includes exponential, trigonometric, and logarithmic terms:
	...
	\subsection{Applications of derivatives}
		...
		\subsubsection{Tangent lines}
			...
			the line that passes through the point $(x_0, f(x_0))$
			and has  the same slope as the function at that point.
			The tangent line to the function $f(x)$
			at the point $x=x_0$ is given by the equation
			\[
			...
			%	we'll learn how to build an approximation $T_n(x)$
	...
	\label{derivatives:optimization}
		
		We're often interested in finding the values $x$ where some function $f(x)$
		...
		For example,
		let's look at the graph of the function $f(x)=x^2$
		...
		\begin{enumerate}
			...
			\item[(C)]	The graph of the function looks locally like the bottom of a valley at $x=0$.
		...
		the second one based on numerical calculations.
		...
		\subsubsection{Analytical optimization}
			...
			occur at the critical points of the function.
			...
			In contrast,
			a critical point $x_k^*$ where $f^{\prime\prime\!}(x_k^*)< 0$ (negative curvature) is a maximum.			
			These observations lead us to the following analytical procedure
			for finding the minima and the maxima of the function $f(x)$:
			...
			\begin{enumerate}
				...
				\item[(2)]	For each critical point $x_i^*$,
						...
						\begin{itemize}
							\item	If $f^{\prime\prime}(x_i^*) < 0$ then $x_i^*$ is a max (mountain top) 		% MAYBE local max
							\item	If $f^{\prime\prime}(x_i^*) > 0$ then $x_i^*$ is a min (bottom of a valley).	% MAYBE local min
			...
			the value $x^*$ is a \emph{stationary inflection point}
			that is neither a minimum nor a maximum.

			\paragraph{Example 9}

				Let's apply the analytical optimization procedure
				to find the minimum value of the function $q(x)=(x-5)^2$.
				The derivative of $q(x)$ is $q^{\prime\!}(x) = 2(x-5)$.
				Next,
				we find the critical point by solving the equation $q^{\prime\!}(x)=0$,
				which has a single solution $x^*_1 = 5$.
				Is the critical value $x^*_1 = 5$ a min or a max?
				To find out,
				we check the sign of the second derivative at $x^*_1$.
				We see $q^{\prime\prime\!}(x^*_1) = 2 > 0$,
				so $q(x)$ looks like the bottom of a valley near $x=x^*+1$.
				This tells us $x^*_1 = 5$ is a minimum.
			...
			\paragraph{Example 10}
				...
				For the critical point $x^*_1 = \frac{1}{3}$,
		...
		\subsubsection{Numerical optimization}
			...
			which allows us to find the minimum of any function.
			...
			and an initial value $x_0$ where we start the minimization.
			The procedure visits the points $x_1$, $x_2$, $x_3$, etc.,
			by repeatedly taking steps
			in the direction opposing the derivative at the current $x$.
			The formula $x_{i+1} = x_i - \alpha f^{\prime\!}(x_i)$ is used to find the next point,
			where the step size is determined by the parameter $\alpha$
			and the slope of the function.
			When $f^{\prime\!}(x_i)$ is positive,
			we take a step to the left.
			When $f^{\prime\!}(x_i)$ is negative,
			...
			using the value $\tt{x0} = 10$ as the starting point of the algorithm.
			...
			The optimization procedure found the values $x=5.000000000396651$
