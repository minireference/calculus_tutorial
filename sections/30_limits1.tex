...
\label{sec:limits}
	...
	% ALT. Archimedes' approximation for area of a circle
		...
		will become more and more accurate as $n$ becomes larger and larger.
		Archimedes only managed to compute an approximation with a $n=96$ sided polygon,
		...
		Using $n=10K$ we get an approximation to $\pi$
...
% and $\textrm{proc}(n)$ describes the output of this procedure when $n$ steps are used.
	...
	\subsection{Computing limits using SymPy}
		...
		% EXAMPLE  Heaviside
		\noindent
		To calculate the limit form the left or the right of a number,
	...
	\subsection{Applications of limits}
		...
		\begin{itemize}
			...
			\item The derivative function $f^{\prime\!}(x)$
				...
				rate change of the function $f(x)$ at $x$.
			...
			\item The integral $\int_a^b f(x) \, dx$
				...
				we'll learn how to compute integrals by splitting up areas into $n$ rectangules,
