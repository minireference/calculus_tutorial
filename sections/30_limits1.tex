...
\label{sec:limits}
	...
	describes what happens to $f(x)$ when $x$ gets larger and larger.
	% to the function $x$ tends to infinity (gets larger and larger).
	...
	% ALT. Archimedes' approximation for area of a circle
		...
		will become increasingly accurate as $n$ becomes larger and larger.
		...
		the approximation $A(n)$ % to the area of the circle 
	...
	\label{introduction:eulers_number}
		...
		Specifically,
		we want to know what happens if the interest is compounded infinitely often.
...
% and $\textrm{proc}(n)$ describes the output of this procedure when $n$ steps are used.
	...
	\label{limits:limits_to_infinity}
		...
		The function $f(x)$ \emph{converges} to $L$
		...
		\begin{figure}[htb]
			...
			\caption{	The function $f(x)$ oscillates up and down initially,
					but then it ``settles down'' close to the value $L$ for large values of $x$.}
		...
		\subsubsection{Example 3}
			...
			% as $\displaystyle \lim_{x\to \infty} f(x)=0$.
	...
	\label{limits:limits_to_zero}
		...
		describes the behaviour of the function $f$ for values of $x$ very close to $0$.
	...
	\label{limits:limits_to_a_number}
		...
		We use the symbol $\delta$ (the Greek letter \emph{delta})
		to describe a distance that gets smaller and smaller.
	...
	\label{limits:continuity}
		...
		If the function $f(x)$ obeys, $f(a) = \lim_{x\to a} f(x)$,
	...
	\subsection{Computing limits using SymPy}
		...
		To compute limit expressions,
		we use the SymPy function \tt{sp.limit(expr,var,value)},
		which returns $\lim_{\tt{var} \to \tt{value}} \tt{expr}$.
		...
		We first define the symbol \tt{x}
	...
	\subsection{Applications of limits}
		...
		\begin{itemize}
			...
			\item The series $\sum_{k=1}^n a_k$ describes
				...
				we'll learn how to compute infinite series by evaluating limits like $\lim_{n \to \infty}$.
