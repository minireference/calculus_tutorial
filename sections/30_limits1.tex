...
\end{figure}
	...
	\label{limits:continuity}
		...
		Consider for example the \emph{Heaviside step function},
...
% /SHORTEN
	...
	\subsection{Computing limits using SymPy}
		...
		\noindent
Note we use the symbol \texttt{oo} (two lowercase \texttt{o}s) to denote infinity $\infty$.
		...
		We provide the expression \tt{1/x} as the first first argument to the function \tt{sp.limit},
		then specify the variable \tt{x} the value \tt{sp.oo}.
		...
		illustrated in Figure~\ref{fig:limits_examples}~(c).
