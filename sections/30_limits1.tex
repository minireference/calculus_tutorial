...
\label{sec:limits}
	...
	\label{limits:limits_to_infinity}
		...
		We can use a limi expressions to describe
		...
		\subsubsection{Example 3}
			...
			Limits are useful because they allow us describe this tendency.
	...
	\label{limits:limits_at_a_point}
		...
		we imagine a small distance $\delta$ (the Greek letter \emph{delta})
	...
	\subsection{Computing limits using SymPy}
		...
		and defining the symbolic variable $\tt{n} = n$,
		...
		so that we can the expression $\tt{1/x} = \frac{1}{x}$
		that we evaluate using the function \tt{sp.limit}:
		...
		\noindent
		This results confirms the answer $\lim_{x \to \infty} \tfrac{1}{x}=0$
