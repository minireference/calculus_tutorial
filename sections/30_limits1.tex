...
\label{sec:limits}
	...
	% For example
	The shorthand ``$\lim$'' is used to denote limit expressions.
...
% REDO FOCUSSING ON lim n -> infty

	\begin{itemize}

% MOVE TO {limits:limits_to_infinity}
		% ASYMPTOTICS
...
% MOVE TO {limits:limits_to_a_number}
		% INFINITELY SMALL
		\item	$\displaystyle  \lim_{\delta \to 0} g(a+\delta)$:
			limit expression that describes the value of the function $g$
			as the input approaches $a$ from the right.
			The number $\delta$ (the Greek letter \emph{delta}) usually describes a small distance,
			and the limit as delta goes to zero,
			$\lim_{\delta \to 0} g(a+\delta)$
			describes the behaviour of the function $g$ for infinitely close to the value $a$.
	...
	\noindent
	Let's look at an example of a math procedure with infinite number of steps.
...
% PASTE AND MERGE EXAMPLE FROM INTRO
...
with compounding is performed infinitely often.
	...
	\subsection{Example: the area of a circle}

		Let's look at another example of a procedure with infinite number of steps
		that was invented by Archimedes,
		one of the calculus OGs.
		...
		by splitting them up into triangular slices,
		and computing the area of these slices using the formula for the area of a triangle $\frac{1}{2}bh$.
		...
		The total area of the polygon with $n$ sides is:
		\[
			 2n \cdot \tfrac{1}{2}ab 
		...
		Here is the code that computes the approximations to the area of a circle % of radius $r=1$
		with polygons with more and more sides.
		...
		\noindent
		To calculate the limit as $n$ goes to infinity,
		we can perform the symbolic math calculation using SymPy:
...
%			(see \href{http://bit.ly/1jGU1Kz}{\texttt{bit.ly/1jGU1Kz}}).
	...
	\label{limits:continuity}
		...
		Geometrically,
		the graph of the function at $x=a$ is ``smooth'' and doesn't make a sudden jump there.
		We say a function is \emph{continuous on the interval} $[a,b]$
		if is continuous for values in that interval.
		Intuitively,
		continuity means the function's graph looks like a smooth curve
		that we can draw in a single stroke without lifting the pen.
		...
		\subsubsection{Example 3}
			...
			Consider the \emph{Heaviside step function},
			...
			\noindent
			The Heaviside function is zero for all negative values of $x$,
			then suddenly jumps to one at $x=0$,
			as illustrated in Figure~\ref{fig:limits_examples}~(c).
			The limit as $x$ approaches $x=0$ from the left
			has the value $\lim_{x\to 0^-} H(x) = 0$.
			The limit as $x$ approaches $x=0$ from the right is $\lim_{x\to 0^+} H(x) = 1$.
			Since the two limits are different,
			we know the function is discontinuous at $x=0$.
...
% /SHORTEN
	...
	\subsection{Computing limits using SymPy}
		...
		The SymPy function \tt{sp.limit(expr,var,value)} allows us to compute the limit
		of the expression \tt{expr}, as the variable \tt{var} approaches \tt{value}.
		Let's use SymPy to revisit the limit calculations we saw above.
		

		% EXAMPLE  e
		Recall the limit expression for Euler's number
		...
		then calling \tt{sp.limit}.

		\begin{codeblock}[sympy-e-from-limit]
		>>> import sympy as sp
		>>> n = sp.symbols("n")
		>>> limit((1+1/n)**n, n, sp.oo)
		E
		\end{codeblock}
		
		\noindent
		SymPy produced the exact value $\tt{E} = e = 2.718281828\ldots$.
		% and not an approximation.
		Note we use the symbol \texttt{sp.oo} (two lowercase \texttt{o}s) to denote infinity,
		which is a clever name since it resembles the infinity symbol $\infty$.


		% EXAMPLE 1/x
		Let's now look at the code for computing the limit $\lim_{x \to \infty} \tfrac{1}{x}$.
		We define the symbol \tt{x}
		then call the function \tt{sp.limit} to evaluate the limit of the expression $\tt{1/x} = \frac{1}{x}$
		as $\tt{x}=x$ goes to infinity $\tt{sp.oo}=\infty$.
	...
	\subsection{Applications of limits}
		...
		\label{limits:limits_for_derivatives}
			...
			\]
			We'll continue the discussion of derivatives in Section~\ref{sec:derivatives}.


		\subsubsection*{Limit for integrals}
		\label{limits:limits_for_integrals}

			One way to approximate the area under the graph of the function $f(x)$ between $x=a$ and $x=b$
			is to split the area into $n$ rectangular strips of width $\Delta x = \frac{b-a}{n}$ and height $f(x)$,
			...
			number of rectangles of infinitely small width:
			{ \small
			\[ 
			  \!\!\int_a^b\!\!\!f(x) \, dx
			  = \!A_f(a,b)
			...
			%	which we'll discuss
			We'll have a lot more to say about integrals in Section~\ref{sec:integrals}.
		...
		\label{limits:limits_for_series}
			...
			For example,
			the sum of the first $n$ terms of the geometric series $a_k= r^k$
			corresponds to the following expression:
			\[
				\sum_{k=0}^n r^k = 1 + r + r^2 + r^3 + \cdots + r^n.
			\]
			The \emph{series} $\sum a_k$
			is defined as the limit $n\to \infty$ of the above expression.
			When $|r|<1$,
