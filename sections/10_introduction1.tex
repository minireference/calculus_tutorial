...
\section{Introduction}
	...
	which tell us how functions \emph{change} over time,
	and \emph{integrals},
	...
	\subsection{Example: car trip}
	\label{introduction:car_example}
	
		Suppose you're driving to see a friend in a neighbouring town.
		The total length of the journey is 27000~m (27~km) along a straight road.
		We can describe your position over time as a function $x(t)$,
		and use the car's trip odometer to measure the distance travelled.
		You start your journey at $t=0$
		when the trip odometer reads $x(0)=0$~m.
	
		\subsubsection{Velocity}
		\label{introduction:car_velocity}
	
			The \emph{derivative function} $x^{\prime\!}(t)$,
			pronounced ``\:\!\!$x$ prime,''
			describes how the function $x(t)$ changes over time.
			In this example $x^{\prime\!}(t)$ is the car's \emph{velocity}.
			If your current velocity is $x^{\prime\!}(t)=15$~m/s,		%	15 m/s=54 km/h
			then the car's position $x(t)$ is increasing by $15$~m each second.
			If you maintain this velocity,
			the position will increase at a constant rate:
			$x(0)=0$~m, $x(1)=15$~m, $x(2)=30$~m, % $\ldots$, $x(100)=1500$~m,
			and so on until $t=1800$~s % ($30$~min)
			when you'll have travelled $1800\times 15 = 27000$~m
			and reached your destination.
	
			To estimate the time remaining in the trip,
			assuming the velocity $x^{\prime\!}(t)$ stays constant,
			you can divide the remaining distance
			by the current velocity:
			\[
				\text{time remaining at } t
					\; \; =\; \frac{ 27000 - x(t) }{ x^{\prime\!}(t) } \;\;\; \textrm{s}.
			\]
			The bigger the derivative $x^{\prime\!}(t)$,
			the faster you'll arrive.
			If you drive two times faster,
			the time remaining will be halved.
	
	
		\subsubsection{Inverse problem}
		\label{introduction:car_inverse_problem}
	
			Imagine that the car's odometer is busted
			and you don't have any way
			to measure the distance $x(t)$ you have travelled over time.
			...
			then you know that the car's position is increasing at the rate of $15$~m each second.
			We can describe the total distance travelled until time $t=\tau$ (the Greek letter \emph{tau})
			as the \emph{integral} of the velocity function $x^{\prime\!}(t)$
			between $t=0$ and $t=\tau$:
			\[
				x(\tau) \; = \; \int_{t=0}^{t=\tau}  x^{\prime\!}(t)\, dt.
			\]
			The integral symbol $\int$ is an elongated $S$ that stands for \emph{sum}.
			To calculate the total distance travelled,
			we imagine splitting the time between $t=0$ and $t=\tau$
			into short time intervals.
			During short time interval $dt$ seconds,
			the position increases by $x^{\prime\!}(t)dt$~m,
			which is the product of the velocity $x^{\prime\!}(t)$ (measured in m/s),
			and the length of the time interval $dt$ (measured in~s).
	...
	shows that calculus concepts
	are not theoretical constructs reserved only for math specialists,
	but common ideas you encounter every day.
	The derivative $q^{\prime\!}(t)$ describes the rate of change of the quantity $q(t)$.
	...
	\subsection{Doing calculus: then and now}
		...
		to give you a complete picture of the tools available for doing calculus.
		...
		\subsubsection{Symbolic calculations using SymPy}
			...
			% CHECK ANSWERS
			You can use SymPy to check the answers you obtain
			from pen-and-paper calculations
			or invent new practice problems.
			% DON'T CHEAT
			% like using ChatGPT
			...
			%
			%	\noindent
			%	We can also ask SymPy to \emph{solve} the equation \t{expr = 0},
		...
		\subsubsection{Numerical computing using NumPy and SciPy}
			...
			and instead represent $\sqrt{2}$ approximately as the floating-point number
			...
			we gain many times over in computational power:
	...
	\subsection{Applications of calculus}
		...
		% machine learning,
		business, economics and other domains that require quantitative analysis.
		...
		and we use derivatives and integrals to do useful calculations based on these functions.
		For example,
		...
		the calculus techniques you learn
