%!TEX root = ../calculus_tutorial.tex
	...
	\label{integrals:techniques_of_integration}
		...
		We're getting into the fourth act of the calculus show,
		and I want to remind you that you can ``tap out'' at any time.
		The material in this act is some of the most boring stuff.
		% that  only for students who are currently taking CALC~II class.
		If you're taking an integral calculus class, %  (a.k.a. CALC 102, Integral calculus),
		then you need to know this stuff,
		because it is going to be your final exam.
		Everyone else
		should feel free to skip ahead to the next section.		% MAYBE: explicit FWD reference
		...
		\subsubsection{Substitution trick}
			...
			just like the search-and-replace operation in a text editor.
			Because we're doing the substitution ``inside'' an integral operation,
			we must change the limits integration (from $a$ and~$b$ to $u_a$ and $u_b$),
			...
			\begin{enumerate}
				...
				\item	Compute $u^{\prime\!}(x)$ and replace $dx$ with $\frac{1}{u^{\prime\!}(x)}du$.
			...
			\paragraph{Example 14}
				...
				Let's use the substitution trick to compute the integral $\int_a^b \frac{1}{x - \sqrt{x}} \, dx$.
				...
				and also replacing $dx$
				with the equivalent expression involving $du$,
				as using the new limits of integration.
				...
				$f(u) = \frac{1}{u-1}$,
				is similar to the function $f(u) = \frac{1}{u}$
				whose antiderivative is $\ln(u)$.
				Replacing $u$ by $u-1$
				gives us the following antiderivative formula $\int \frac{1}{u - 1} \, du = \ln(u-1)$,
			...
			%	The substitution trick comes from the chain rule for derivatives
			%	$\left[ F(u(x)) \right]^\prime = F^{\prime\!}(u(x))u^\prime(x)$.
			%	We can use substitution only when computing the integral of a function
			%	that has the special structure $f(u(x))u^{\prime\!}(x)$.
		...
		\subsubsection{Integration by parts}
			...
			and $g^{\prime\!}(x)$ is the derivative of another function.
			...
			\noindent
			Essentially,
			we're replacing the work of integrating $f(x)g^{\prime\!}(x)$
			with the work of integrating $f^{\prime\!}(x)g(x)$,
			which can sometimes be simpler to calculate.
			Specifically,
			for integration by parts trick to be useful,
			we want $f^{\prime\!}(x)$ to be a simpler function than $f(x)$,
			and $g(x)$ not to be much more complicated than $g^{\prime\!}(x)$.
			...
			\paragraph{Example 15}
				...
				\begin{align*}
					\int_0^5 \underbrace{x}_{f(x)} \underbrace{e^x}_{g^{\prime\!}(x)} \, dx
						...
						&= \big[ 5 e^5 \qquad\; - 0e^0 \big] \qquad \; - \big[  e^5 - e^0 \big] 						\\
						&= 5 e^5 - e^5 + 1  = 4e^5 + 1.
				\end{align*}
		...
		\subsubsection{Other tricks}
			...
			%	Mathematicians and physicists have come up with hundreds of formulas and tricks
			...
			fractions that involve $x^2 + a^2$, etc.
			There is an entire course called integral calculus
			that is dedicated to learning integration tricks.
			If you plan to study physics or engineering,
			you should definitely take this course to learn all the integration tricks.
			See Section~5.15 in the \textbf{No Bullshit Guide to Math and Physics}
			for more on this.
			% details about this topic.

			Not all functions are \emph{integrable}.
			There are functions that don't have an antiderivative function,
			...
			our only option is to use numerical integration:
			the split-the-area-into-rectangular-strips procedure.
