...
\vspace{-3mm} % LAYOUT
	...
	\label{integrals:computing_integrals_numerically}

\vspace{-2mm} % LAYOUT

		Computing the integral $\int_{a}^b f(x)dx$ \emph{numerically}
		means using a computer to compute the Riemann sum approximation to $A_f(a,b)$
		by splitting the region into many (think millions) of strips,
		computing the areas of each strip,
		then adding up the areas to get the total area under the graph of $f(x)$.
		...
		then evaluate this expression for large values of $n$.
		...
		and we sum together the $10$ area contributions
		...
		\[ 
			...
			= 4.92.
		\]
		Looking at Figure~\ref{fig:riemann_sum_n10_n20}~(a),
		we can clearly see that the area of the rectangles
		underestimates the true area under the graph,
		but let's keep going.
		We have to trust the process:
		the quality of the approximation will improve
		when we split the region into thinner and thinner strips.

		The procedure we used for $n=10$ works more generally for any $n$.
		In the general case,
		the rectangles have width $\Delta x = \frac{b-a}{n} = \frac{2}{n}$,
		which gets smaller and smaller as $n$ grows.
		The $x$-coordinates of the right endpoints of the $n$ rectangles
		are located at $[\Delta x, 2\Delta x, 3\Delta x, \ldots, (n-1)\Delta x, n\Delta x]$.
		...
		we sum together the individual height-times-width
		...
		and $k$ goes from $1$ to $n$.
		Mathematicians use the symbol $\sum$
		(the capital Greek letter \emph{sigma})
		to describe long summations.
		The approximation to the area under $h(x)$ between $x=a$ and $x=b$
		using $n$ rectangular strips corresponds to the following sum:
		$A_h(0,2) \approx \sum_{k=1}^{n} h(k\Delta x)\,\Delta x$.
		...
		and the label $k=n$ tells us where to stop the summation.
		...
		\[
			...
			\Delta x = \tfrac{b-a}{n}.
		\]
		This is known as the \emph{Riemann sum} formula for approximating areas.
		We'll now turn the Riemann sum formula into a Python procedure
		...
		\noindent
		The code implements the operations
		described by the summation $A_f(a,b) \approx \sum_{k=1}^{n} f(a + k\Delta x)\,\Delta x$.
		We first compute the width of the rectangles $\code{dx} = \Delta x = \frac{b-a}{n}$,
		and create the list \tt{xs}
		that contains the $x$-coordinates of the right endpoints of the rectangles,
		$\tt{xs} = [a + \Delta x, a + 2\Delta x, a + 3\Delta x, \ldots, a + n\Delta x]$.
		We then evaluate the function \tt{f} at these $x$-values
		to obtain $\tt{fxs} = [f(a + \Delta x), f(a + 2\Delta x), f(a + 3\Delta x), \ldots, f(a + n\Delta x)]$.
		...
		which we \tt{return} as the output of the procedure.
		%	The definition of the \tt{integrate} procedure specifies a default value $n = 10\,000$,


		\subsubsection{Example 13 continued}
			...
			To calculate the $n=10$ approximation to the area under the graph of $h(x)$ between $x=0$ and $x=2$,
			...
			computed using $n=1M$ rectangles is accurate to $4$ decimals.
			The exact value of the area $A_h(0,2)$ is
			$\frac{16}{3} = 5\frac{1}{3} = 5.\overline{3} = 5.33333333333333\ldots$.
	...
	\subsection{Formal definition of the integral}
	% Formal definition of the integral as the limit of a Riemann sum

		In the limit as the number of rectangles $n$ approaches $\infty$,
		the approximation to the area under the graph of $f(x)$
		becomes \emph{arbitrarily close} to the true area.
		...
		We previously defined the integral $\int_{a}^{b}f(x)\,dx$ geometrically
		as the area under the graph of $f(x)$, % $A_f(a,b)$,
		but now you know the formal math definition for the integral that mathematicians use.
		...
		in both cases we evaluate $f$ at different $x$-values,
		multiply by a width,
		...
		which corresponds to rise-over-run calculation with an infinitely short run $\delta$.
		...
		are essential tools that allow us to define these calculus operations on functions.
