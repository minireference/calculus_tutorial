...
%	For the purpose of learning probability and statistics,
...
%	as we'll show in the next two sections.
	...
	\label{integrals:scipy}
		...
		that we defined in Section~\ref{integrals:computing_integrals_numerically}
		is a useful teaching tool,
		but it is much too slow to use for practical scientific computing tasks.
		% that require computing integrals thousands of times.
		The function \tt{quad} from the \tt{scipy.integrate} module
		is a much more powerful tool for computing integrals numerically.
		The name \tt{quad} is short for ``quadrature,''
		which is a historical name for integrals.
		% finding the area under the graph of a function.
		% performed by splitting it into small sections % (squares)
		% and adding up the total area of the sections.

		Let's revisit the examples 11, 12, and 13 using the function \tt{quad}.
		...
		\noindent
		The function \tt{quad} returns two numbers as output: $\big(A_f(0,5),\epsilon\big)$.
		...
		%
		Since we're usually only interested in the area $A_f(0,5)$
		and not the precision $\epsilon$,
		we often select the first element of \tt{quad}'s output.
		...
		\noindent
		The answers we get using \tt{quad}
		match the results we obtained earlier.
		...
		can be approximated numerically using one or two lines of Python code.
		Specifically,
