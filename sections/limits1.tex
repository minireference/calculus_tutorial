...
\label{sec:limits}
	...
	\subsection{Example}
		...
		The formula for the area of a regular polygon with $n$ sides that fits inside a circle of radius $r$ is...
		
		
		% FIGURE hexagon, octagon, dodecagon side by side with triangle + labels on octagon 

		\begin{figure}[htb]
			\centering
			\includegraphics[width=0.5\textwidth]{figures/calculus/inscribed-hexagon-octagon-dodecagon.png}
			\caption{	A hexagon, an octagon, and a dodecagon inscribed inside a circle with radius $r=1$.}
...
%		\begin{center}
%		\includegraphics[width=0.3\textwidth]{figures/calculus/octagon_inscribed_in_circle.pdf}
%		\end{center}		
...
TODO: introduce limit usage



		For example,
		to compute the limit of the fraction $\frac{2x+1}{x}$ as $x$ goes to infinity,
		...
		\noindent
		To specify $\infty$ we use the \tt{oo} (two lowercase letters ``o'').
...
We'll explain both of these in the next sections.
		...
		\label{limits:limits_for_series}
			...
			You'll find more about series in Section~\ref{sec:sequences_and_series}.
	

	% LEAD OUT	
	In the remainder of this tutorial we'll learn about derivatives, integrals, and series
	and limit expressions will help us make precise statements in each case.
