...
\section{Math prerequisites}
	...
	%Before we get started with calculus,
	Let's start with a review of some key ideas from high school math,
	which we'll need as building blocks for calculus.
	...
	\subsection{Set notation}
		...
		\begin{itemize}
			...
			\item $\{ \textrm{  definition  } \}$:
				...
				The definition in the curly brackets is either a description of the set's contents,
				or a list of the elements in the set.
				% The math symbol ``$|$'' is shorthand for the phrase ``such that'' and it is often used in definitions.

			\item $\mathbb{N}$: the set of natural numbers $\mathbb{N} = \{0,1,2,3,4,5,\ldots\}$.

			\item $\mathbb{N}_+$: the positive natural numbers $\mathbb{N}_+ = \{1,2,3,4,5,\ldots\}$.
			...
			%	\item $\mathbb{Z}$: the set of integers $\mathbb{Z} = \{\ldots,-2,-1,0,1,2,3,\ldots\}$
			%
			%	\item $\mathbb{Q}$: the set of rational numbers,
			%		$\mathbb{Q} = \left\{  \frac{m}{n} \; \Big| \; m \in \mathbb{Z}, \; n \in \mathbb{N}, \; n \neq 0  \right\}$.
		...
		\noindent
		We can use the \emph{set-builder} notation $\{\, \cdot \; | \;  \cdot \, \}$ to define new sets.
		...
		followed by the conditions that identify the elements of the set.
		For example,
		the set of nonnegative real numbers $\mathbb{R}_+$
		is defined as ``all real numbers $x$ such that $x \geq 0$,''
		...
		\subsubsection{The number line}
			...
			All the numbers you will run into when doing math
		...
		\subsubsection{Infinity}
			...
			We use $\infty$ to represent an infinitely large quantity,
			that is greater than any number you can think of.
			Geometrically speaking,
			you can imagine that the number line extends forever to the right toward infinity,
			...
			Infinity is a key concept in calculus,
			so it's important that we develop a precise language to talk about
			infinitely large numbers and procedures with an infinite number of steps,
			which we'll do in Section~\ref{sec:limits}.
			...
			% COUNTABLE INFINITY
			%	Consider the set of natural numbers $\mathbb{N} = \{0, 1, 2, 3, 4, 5, 6, \ldots \}$.
	...
	%		shows an illustration of the interval $[2,4] = \{ x \in \mathbb{R} \;|\; 2 \leq x \leq 4 \}$,
	...
	%			\vspace{-3mm}
	%			\caption{	The interval $[2,4] \protect = \{ x \in \mathbb{R} \; | \; 2 \leq x \leq 4 \}$
	...
	\subsection{Functions}
		...
		A \emph{function} is a mathematical object
		that takes numbers as inputs and produces numbers as outputs.
		The output of the function $f$ for the input $x$ is denoted $f(x)$.
		We can define a function by writing an expression
		for the output of the function when the input is $x$.
		For example,
		the quadratic function is defined as $f(x) \eqdef x^2$.
		We use the ``is defined as'' symbol $\eqdef$
		instead of the regular equals sign $=$ to indicate this is a definition.
		The function $f(x)$ takes any number $x$ as input,
		and produces the square of this number as output.
		For example,
		when the input is $x=3$,
		the output of the function is $f(3) = 3^2 = 9$.
		...
		\noindent
		Note the Python syntax for evaluating the function \tt{f} for the input \tt{3}
		...
		\subsubsection{Function graphs}
			...
			Function graphs are an essential tool in calculus.

			We can use Python to obtain the graph of the function \tt{f} that we defined earlier.
			...
			that contains 1000 $x$-values that range between $x=-3$ and $x=3$.
			We then evaluate the function \tt{f} for all inputs \tt{xs}
			and store the outputs in an array called \tt{fxs}.

			\begin{codeblock}[np-linspace-xs-and-fxs]
			>>> import numpy as np
			...
			\noindent
			At this point,
			the array \tt{xs} contains $1000$ $x$-inputs,
			while the array \tt{fxs} contains the corresponding $1000$ outputs of $f(x)$.
			To generate the graph of $f(x)$,
			we just need to trace a line passing through the $1000$ coordinate pairs $(x, f(x))$.
			We can do this by importing the \tt{seaborn} module (alias \tt{sns})
			and calling the function \tt{sns.lineplot} as shown below.

			\begin{codeblock}[sns-lineplot-xs-fxs]
			...
			\begin{figure}[htb]
				...
				\vspace{-3mm}
				\caption{	Graph of the function $f(x) = x^2$ from $x=-3$ to $x=+3$.
						The graph of the function $f$ passes through the coordinate pairs $(x,f(x))$
						for all $x$-values between $x=-3$ and $x=3$.}
				\label{fig:graph_of_function_f_eq_x2}
			\end{figure}

			\noindent
			We'll use the combination of \tt{np.linspace},
			function evaluation,
			and \tt{sns.lineplot} when we want to plot a function's graph.
		...
		\subsubsection{Inverse functions}
			...
			\begin{figure}[htb]
				...
				\caption{The inverse $f^{-1}$ undoes the action of the function $f$.}
		...
		\subsubsection{Function properties}
			...
			and its image is $\mathbb{R}_+$ (nonnegative real numbers),
			so we say $f$ is a function of the form $f \colon \mathbb{R} \to \mathbb{R}_+$.
	...
	\subsection{Inventory of functions}
		...
		determines which English sentences you'll be able to read and understand.
		Figure~\ref{fig:panel_function_graphs1}
		shows the graphs of six important functions
		that are often used for mathematical modelling.

		\begin{figure}[htb]
			\centering
			\includegraphics[width=0.99\columnwidth]{figures/calculus/panel_function_graphs1.pdf}%
			\vspace{-2mm}
			\caption{	Graphs of six math functions that you should know about.}
		...
		\subsubsection{Linear function}
			...
			where the output value $f(x)$ is \emph{proportional} to the input value $x$,
			and the constant of proportionality is $m$.
			Geometrically,
			$m$ is the slope in the graph of $f(x)$.
			Figure~\ref{fig:panel_function_graphs1}
			shows the graph of $f(x) \eqdef x$ which is the linear function with $m=1$
			for which the output $f(x)$ is equal to the input $x$.
			%
			More generally,
			we can define the line $f(x) \eqdef mx+b$,
			where $m$ describes the slope of the line,
			and $b$ is the initial value of the function when $x=0$.
		...
		\subsubsection{Quadratic function}
			...
			The outputs of the quadratic function are always nonnegative numbers
		...
		\subsubsection{Polynomial functions}
			...
			We can combine different powers of $x$
			to obtain the polynomial $f(x) \eqdef a_2x^2 + a_1x + a_0$,
			where $a_2$, $a_1$, $a_0$ are arbitrary constants.
			This is called a second degree polynomial,
			since the highest power of $x$ is $x^2$.
			%
			The general equation for a polynomial function of degree $n$ is
			\[
				P_n(x) = a_0 + a_1x + a_2x^2 + a_3x^3 + \cdots + a_nx^n.
			\]
			The constants $a_0,a_1,a_2,\ldots, a_n$ are called the \emph{coefficients} of the polynomial.
			% A polynomial of degree $n$ has  $n+1$ coefficients: .
			Polynomials are a very useful family of functions,
			because they can take on different shapes depending on the choice of the coefficients.
			In Section~\ref{series:taylor_series},
			we'll learn how to obtain polynomial approximations to any function $f(x)$.
			% MAYBE: $P_n(x) \approx f(x)$.
		...
		\subsubsection{Exponential function}
			...
			is defined as $f(x) \eqdef e^{x} = \exp(x)$,
			where $e=2.7182818\ldots$ is Euler's number.
		...
		\subsubsection{Absolute value function}
			...
			Geometrically,
			$|x|$ corresponds to the distance between $x$ and the origin of the number line.
			%
		...
		\subsubsection{Square root function}
			...
			Note that $-\sqrt{c}$ is also a solution to $x^2=c$.
			%
			...
			we get back to $x$:
			$\big(x^{\frac{1}{2}}\big)^2 = x^{\frac{2}{2}} = x^{1} = x$.
			%
			In addition to the \emph{square} root,
			there is also the \emph{cube root} function $f(x) = \sqrt[3]{x}  = x^{\frac{1}{3}}$,
			which is the inverse function for the cubic function $f(x)=x^3$.
			For example,
			$\sqrt[3]{8}=2$ since $2\times2\times2=8$.
		...
		\subsubsection{Logarithmic function}
			...
			passes through the following coordinate pairs:
			$(\frac{1}{e^2},-2)$, 
			$(\frac{1}{e},-1)$, 
			$(1,0)$, 
			$(e, 1)$, 
		...
		\noindent
		There are many other functions worth knowing about,
		but if you're familiar with the six functions shown in Figure~\ref{fig:panel_function_graphs1},
	...
	\subsection{Functions with discrete inputs}
		...
		We often express sequences by writing explicitly
		the first few values of the sequence $[a_0, a_1, a_2, a_3, \ldots]$,
		which correspond to evaluating $a_k$ for $k=0$, $k=1$, $k=2$, $k=3$, etc.
		We'll learn more about sequences and their properties in Section~\ref{sec:sequences_and_series}.
	...
	\subsection{Trigonometric functions}
		...
		where $\cos\theta$ and $\sin\theta$ are the trigonometric functions.
		...
		\begin{figure}[htb]
			...
			\vspace{-2mm}
			\caption{	The coordinates of the point on the unit circle are $x=\cos\theta$ and $y=\sin\theta$,
		...
		so we often need to convert angles from degrees to radians. % and back.
		...
		\subsubsection{Sine function}
			...
			We also use the sine function to find the $y$-component of a unit length,
		...
		\subsubsection{Cosine function}
			...
			The cosine function represents the horizontal position of a point on the unit circle,
			and the $x$-component of a unit length,
			as illustrated in Figure~\ref{fig:trigonometric_functions_as_point_or_as_triangle}.
			...
			%		The graph of the tangent function is shown in Figure~\ref{fig:panel_function_graphs2}~(c).
			%
		...
		\begin{figure}[htb]
			...
			\vspace{-2mm}
			\caption{	Graphs of the trigonometric functions $\sin(\theta)$ and $\cos(\theta)$ showing two full cycles.
					The graphs represent the vertical and horizontal position of a point that completes two full
					circles around the unit circle.}
			\label{fig:panel_function_graphs2}
		\end{figure}


		% APPLICATIONS
		We use the trigonometric functions $\sin(\theta)$ and $\cos(\theta)$
		to compute components of vectors in different directions.
		The sine and cosine functions are also used to describe waves and periodic motion in physics.
		In this tutorial,
		we won't discuss these applications too much,
		but you need to know about the trigonometric functions' graphs and rates of change.
