...
Note that infinity is not a number but a \emph{process}.
We denote $+\infty$ the prices of adding $+1$ forever.
More on this in Section~\ref{sec:limits}.

Remember what we said in the introduction,
$\infty$ is not a number but a process.
			% ALT. not a destination you can get to
			
TODO: INSERT INFINITY FROM INTRO
...
% /NEEDED?  USED?
	...
	\subsection{Functions}
		...
		\subsubsection{Function graphs}
			...
			as illustrated in Figure~\ref{fig:graph_of_function_f_eq_halfx2}.
			%
			...
			to plot the graph of the function $f(x) \eqdef \frac{1}{2}x^2$ that we defined in the previous code block.
			...
			\begin{figure}[htb]
				...
				\caption{	Graph of the function $f(x)=\frac{1}{2}x^2$ from $x=-3$ until $x=+3$.
						...
						for all $x$-values in the interval $[-3,3]$.}
		...
		\subsubsection{Inverse functions}
			...
			If we apply the inverse function $f^{-1}(x) = \sqrt{2x}$ to $4.5$,
			we get back to the number $3$ that we started from
			$f^{-1}(4.5) = \sqrt{2 \cdot 4.5} = \sqrt{9} = 3$.
