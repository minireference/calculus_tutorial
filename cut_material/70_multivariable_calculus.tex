...
\label{sec:multivariable_calculus}
	...
	\subsection{Multivariable functions}
		...
		We trace \emph{level curves} on this surface
	...
	\subsection{Partial derivatives}
		...
		Taking the derivative of a multivariable function with respect to one of its input variables
		is called a \emph{partial derivative}
		...
		The operation $\frac{\partial}{\partial x}f(x,y)$
		describes taking the derivative of $f(x,y)$ with respect to the input variable $x$,
		...
		\subsubsection{Example}
			...
			%	\]
			We can use the derivative formulas from Table~\ref{table:derivatives}---you
			just have to remember which variable you're differentiating
			and which variable stays constant.
	...
	\subsection{The gradient operator}
		...
		``uphill'' direction at the surface of $f(x,y)$ at the point $(x,y)$.
	...
	\subsection{Applications of multivariable calculus}
		...
		Indeed,
		the \emph{gradient descent algorithm} can be used to optimize functions
		...
		Most modern machine learning models
		``learn'' the model parameters by minimizing the value of some multivariable function
		by repeatedly taking steps in the ``downhill'' direction,
